...
module CKB
  class MultiSignConfiguration
    ...
    def self.from_public_keys(require_n:, threshold:, public_keys:, since:)
      pubkey_hashes = public_keys.map do |pubkey|
        CKB::Utils.bin_to_hex(CKB::Blake2b.digest(CKB::Utils.hex_to_bin(pubkey))[0..19])
      end
  ...
  class MultiSignWallet
    ...
    def interactive_generate_tx(target_address, capacity, data: "0x", fee: 0)
      parsed_address = AddressParser.new(target_address).parse
      raise "Right now only supports sending to default single signed lock!" if parsed_address.address_type != "SHORTSINGLESIG"
      ...
      private_key_length = configuration.threshold
      empty_signature = Utils.bin_to_hex(("\x00" * 65) * private_key_length)
      emptied_witness = tx.witnesses[0].dup
      emptied_witness.lock = Utils.hex_concat(configuration.serialize, empty_signature)
      emptied_witness_data_binary = Utils.hex_to_bin(
        Serializers::WitnessArgsSerializer.from(emptied_witness).serialize)
      ...
      signatures = []
      private_key_length.times.each do |i|
        puts "Please enter signature #{i + 1}:"
        signature = $stdin.readline
        puts "Got signature: #{signature}"
        signatures << signature.strip
      end
    ...
    def send_capacity(target_address, capacity, private_keys: nil, data: "0x", fee: 0)
      if private_keys
        tx = generate_tx(target_address, capacity, private_keys, data: data, fee: fee)
      else
        tx = interactive_generate_tx(target_address, capacity, data: data, fee: fee)
      end
